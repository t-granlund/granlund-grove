...
- [ ] `[Tyler]` **Always Use HTTPS** ON; verify headers/contact-send/404 live post-deploy.
...
- [ ] `[Tyler]` **Native rate-limit binding** on `/api/contact` (needs deploy to validate).
- [ ] `[Tyler]` **www→apex Redirect Rule**, **Polish=Lossy+WebP**, **Cache Rules** `/assets/*` `/img/*` `/resume/*`.
- [ ] `[Tyler]` **DMARC record** `p=none` + Min TLS 1.2 + Auto HTTPS Rewrites.
